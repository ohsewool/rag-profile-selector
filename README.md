...
**건너뛴 검사는 아무것도 확인하지 않은 초록불이다.** 마흔아홉은 새 하나가 숨기에 충분한 수라서, 어느 파일이 몇 개를 어떤 이유로 건너뛰는지를 `conftest.py`가 집합으로 지킨다 — 늘어나도, 이유가 달라도, 고쳐서 줄어들어도 걸린다.

그걸 쓰다 조건이 하나 어긋난 것을 찾았다. 네 파일 중 `test_stratification.py`만 **디렉터리 존재만** 보고 나머지 셋은 그 안에 조문 JSON이 있는지까지 봤다. `fetch`가 중간에 죽어 빈 디렉터리가 남으면 그 파일만 **빈 코퍼스를 상대로 돈다.** 조건과 문장을 `tests/corpus_absence.py` 한 곳으로 모았다.
