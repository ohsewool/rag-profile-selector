...
python3 -m pytest tests/ -q                        # 261 통과 · 25 skip (코퍼스 없이)
python3 scripts/fetch_kr_law_corpus.py             # 한국어 법령 코퍼스 (공공누리 제1유형)
python3 -m pytest tests/ -q                        # 286 tests (코퍼스 받은 뒤 전부 통과)
...
없어서 **25개가 skip된다** — 받아온 법령이 조문 단위로 쪼개졌는지, 식별자가 유일한지,
제7조와 제7조의2가 뭉개지지 않았는지, 그리고 층화 도구가 실제 중복을 재는지 확인하는
것들이다. 위 `fetch_kr_law_corpus.py`를 돌리면 286개가 전부 돈다. 이 두 숫자는 CI가
